...
> 작업 일자: **2026-02-20 ~ 2026-02-21 (KST)**  
> 배치: **#1**
...
### 5-1. 텔레그램 흐름 캡처
- 무엇을 보는지: `ping → pong` 왕복이 정상 동작하는지 확인
- 왜 중요한지: 설정이 맞아도 실제 채널에서 실패하는 경우가 있어 E2E 검증이 필요함
- 재현 명령: 텔레그램에서 `ping` 전송 후 응답 확인

### 5-2. 페이지(설정/배포) 캡처
- 무엇을 보는지: GitHub Pages 배포 상태(빌드 성공/반영 여부)
- 왜 중요한지: 로컬 정상과 배포 정상은 별개라서 공개 링크 상태를 분리 검증해야 함
- 재현 명령: GitHub repo → Settings → Pages / Actions 확인

### 5-3. 터미널 캡처
- 무엇을 보는지: `openclaw gateway probe`, `openclaw status` 결과
- 왜 중요한지: 인증/페어링/연결 문제를 가장 빠르게 분리 진단할 수 있음
- 재현 명령:
...
### 5-4. 코드/명령 스니펫
- 무엇을 보는지: 모델 복귀 및 상태 확인 명령
- 왜 중요한지: 운영 안정 상태로 즉시 롤백할 수 있는 최소 명령셋
- 재현 명령:
...
## 8) 이 배치에서 나눈 대화/의사결정 포인트

- "결제했는데 왜 추가 과금이 붙지?"라는 질문에서 출발해 **구독 경로와 API 경로를 분리**해서 이해함
- 연결 이슈는 모델이 아니라 **pairing/token mismatch** 레이어라는 데 합의함
- API 키가 채팅에 노출됐을 때는 기능보다 보안을 우선해 **즉시 revoke** 처리함
- 블로그 방향은 "일기형"보다 **작업 배치형 기술 회고**로 정함
- 글 스타일은 한국 기술블로그 문체 기준으로, **문제-선택-결정-결과-주의점** 순서를 유지하기로 함
- 앞으로는 캡처(텔레그램/페이지/터미널) + 복붙 가능한 명령 중심으로 남기기로 함
